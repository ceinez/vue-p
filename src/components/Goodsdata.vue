            ...
            <div class="item-box">
                    <div class="gallery-wrapper">
                        <div class="gallery">
                            <div class="thumbnail">
                                <ul>
                                    <li :key="item.id" v-for='(item,index) in item.ali_images' @click=handler(index,item) :class="{on: current_index==index}" >
                                        <img :src="item" >
                                    </li>
                            ...
                            <div class="thumb">
                                <ul>
                                    <li :key="item.id" v-for='(item,index) in item.ali_images' :class="{on:current_index==index}" >
                    ...
                    <div class="banner">    
                        <div class="sku-custom-title">
                            <div class="params-price">
                                <span><em style="font-size:25px;">$</em><i>{{item.price}}</i></span>
                            </div>
                        ...
                        <div class="sku-dynamic-params-panel">
                            <div class="sku-dynamic-params clear">
                                <div class="params-name">Qty</div>
                                <div class="params-detail clear">
                                    <div class="item-num js-select-quantity">
                                    <span class="down"   :class="{'down-disabled':temp<1}" @click="del">- </span>
                                        <span class="num">{{temp}}</span>
                                        <span class="up" :class="{'up-disabled':temp>=item.limit_num}" @click="add">+</span>
                        ...
                        <div class="sku-status">
                            <div class="cart-operation-wrapper clearfix">
                                <span class="blue-title-btn js-add-cart" @click="addToCart(item,temp)"><a>Add to Cart</a></span>
...
export default {
    ...
    methods:{
        ...
        add(){
             this.temp+=1
              if(this.temp>=this.item.limit_num){
                 this.temp=this.item.limit_num
             }
        },
        addToCart(data,temp){
            var datas={data:data,temp:temp}
            this.$store.commit('getAddToCart',datas)
        }
    },
    props:{
        item:Object
