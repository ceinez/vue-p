        ...
        <div class="store-content">
            <div class="cart-box">
                ...
                <div class="cart-inner">
                    <div class="empty-label"  v-if="totalNum<=0">
                        <h3>There are no items in your cart</h3>
                        <router-link class="link" to="/">Buy Now</router-link>
                    </div>
                    <div v-if="totalNum>0">
                        <div class="cart-table-title">
                            <span class="name">Goods Info</span>
                        ...
                        <div class="cart-table">
                            <div class="cart-group">
                                <div class="cart-top-items" :key='item.id' v-for='item in goodsItem' >
                                    <div class="cart-items">
                                        <div class="items-choose">
                                            <span class="blue-checkbox-new  " :class="{'checkbox-on':item.checked}" @click="handlerChange(item.id)">
                                                <a></a>
                                            </span>
                                        </div>
                                        ...
                                        <div class="operation" @click="handlerDel(item.id)">
                                            <a class="items-delete-btn"></a>
                                        </div>

                                        <div class="subtotal">$ {{item.temp*item.price}}</div>
                                        
                                        <div class="item-cols-num">
                                            <div class="select js-select-quantity">
                                                <span class="down"   :class="{'down-disabled':item.temp<=1}" @click="delCart(item.id)">- </span>
                                                <span class="num">{{item.temp}}</span>
                                                <span class="up" :class="{'up-disabled':item.temp>=item.limit_num}" @click="addCart(item.id)">+</span>
                                            </div>
                                        </div>
                ...
                <div class="cart-bottom-bg fix-bottom" v-if="totalNum>0">
                    <div class="cart-bar-operation">
                        <div>
                            <div class="choose-all js-choose-all">
                                <span class="blue-checkbox-new " :class="{'checkbox-on':allCartChacked}" @click="handlerAllChacked(allCartChacked)"><a></a></span>
                              Check All
                            </div>
                            <div class="delete-choose-goods" @click="delSelectedItems">Delete selected items</div>
                        </div>
                    </div>
                    <div class="shipping">
                        <div class="shipping-box">
                            <div class="shipping-total shipping-num">
                                <h4>
                                    Checked <i>{{allChackedNum.temp1}}</i></h4>
                                <h5>
                                    Quantity <i>{{allChackedNum.temp}}</i></h5>
                            </div>
                            <div class="shipping-total shipping-price">
                                <h4>
                                    Total payable：<span>$ </span><i>{{allChackedPrice}}</i></h4>
                                <h5 class="shipping-tips" v-if="allChackedPrice>=200">
                                   免费包邮
                                </h5>
                                <h5 class="shipping-tips" v-else>
                                    满200包邮否则邮费$10
                                </h5>
                            </div>
                        </div>
...
export default {
    computed:{
        ...mapState(['goodsItem']),
        ...mapGetters(['totalNum','allCartChacked','allChackedNum','allChackedPrice'])

    },
    methods:{
      ...mapMutations(['delCart','addCart','delSelectedItems']),
        handlerChange(id){
            this.$store.commit('changeChecked',id)
        },
        handlerAllChacked(allCartChacked){
            this.$store.commit('changeAllChacked',allCartChacked)
        },
        handlerDel(id){
            this.$store.commit('handlerDelData',id)
        }
    }
